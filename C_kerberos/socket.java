...
public class socket {
       ...
       static String ASIP="192.168.43.116";
       static String TGSIP="192.168.43.93";
      // static String VIP="192.168.43.9";
	  static String VIP="192.168.43.116";
			...
			try {
				...
				Au=au.enclosure(Kcv,IDc,source_IP, Time());
					...
					{   
						...
						System.out.println("来自socket的的消息                                     "+str);
...
//
		   ...
		   String Au="";
			...
			try {
				...
				Au=au.enclosure(Kctgs,IDc,source_IP, Time());
					...
					Vsocket(handleTGS.Tv,handleTGS.Kcv);//调用Vsocket
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				try {
	   ...
	   public socket(String ID,String password)//构造函数
	   {
			...
			try {
				...
				TGSsocket(handleC.Ttgs,handleC.Kctgs);//调用TGSsocket
		...
		socket a=new socket("0001","dds123fff");
		}
	}
